...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    # build dataset
    (train_sampler, train_dataloader), (_, test_dataloader),= builder.build_dataset(args, config.dataset.train), \
                                                            builder.build_dataset(args, config.dataset.val)
    (_, extra_train_dataloader)  = builder.build_dataset(args, config.dataset.extra_train) if config.dataset.get('extra_train') else (None, None)
    # build model
    base_model = builder.build_model(config.model)
    if args.use_gpu:
