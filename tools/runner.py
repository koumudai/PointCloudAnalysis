...
def train(args, config, train_writer=None, valid_writer=None):
    ...
    # build dataset
    (train_sampler, train_dataloader), (_, test_dataloader) = builder.dataset_builder(args, config.dataset.train), builder.dataset_builder(args, config.dataset.valid)
    # build model
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        if epoch % args.val_freq == 0 and epoch != 0:
            ...
            metrics = validate(base_model, extra_train_dataloader, test_dataloader, epoch, valid_writer, args, config, logger=logger)
